---
name: sales-contacts
description: Decision maker intelligence and contact strategy for /sales contacts. Use to identify buying committees, org hierarchy, priority contacts, personalization anchors, verified email status, fallback channels, and produce DECISION-MAKERS.md.
---
...
### 1.4 Email Discovery & Validation

Determine whether each priority contact has a usable, validated email address. Email pattern detection is useful evidence, but it is not enough to recommend sending cold outreach to a specific address.
...
**Discovery and validation procedure:**
...
6. Search exact public queries for each priority contact: `"[contact name]" "[company domain]"`, `"[contact name]" email`, and `"[contact name]" "[company name]" "@[domain]"`.
7. Check PDFs, event speaker bios, podcast pages, webinar pages, news articles, and media kits for named-contact emails.
8. If a trusted email verification or enrichment tool is available, use it to verify deliverability before recommending the address.
9. Check MX/domain validity where tooling is available, but do not treat a valid domain as proof that an individual mailbox exists.

**Email confidence labels:**

| Label | Definition | Outreach Use |
|-------|------------|--------------|
| Verified | Exact individual address found in a public source or verified by a trusted enrichment/verification tool | Safe to recommend for email outreach |
| Pattern-derived | Address inferred from a detected company pattern, but not directly found or verified | Do not recommend sending; list only as possible pattern |
| Generic | Department or company mailbox such as info@, sales@, or contact form | Use only as fallback routing, not primary executive outreach |
| Unknown | No usable email evidence found | Use LinkedIn, phone, contact form, or alternate contact |

**Hard rule:** Never present a pattern-derived address as the send-to email. If the best strategic contact does not have a Verified email, search the other priority contacts for a Verified email and recommend the best alternate buying-committee member. If no individual Verified email exists, clearly state that direct email is blocked and recommend non-email channels.
...
| Criteria | Points |
|----------|--------|
| Verified individual email found for primary contact | +12 |
| Verified individual email found for alternate buying-committee contact | +8 |
| LinkedIn profiles found for key contacts | +4 |
| Generic company mailbox or contact form found | +3 |
| Phone number found | +2 |
| Email pattern identified but no address verified | +1 |
...
| **Fallback Channel** | [LinkedIn / phone / contact form / alternate verified contact] |
...
Before recommending an email-led sequence, compare the top 3-5 priority contacts by both buying relevance and email status.

Selection rules:
1. If the best strategic contact has a Verified email, use that person as the primary email target.
2. If the best strategic contact has only Pattern-derived, Generic, or Unknown email status, do not recommend a guessed address.
3. Search the remaining priority contacts for a Verified individual email.
4. If an alternate contact has a Verified email and a plausible buying role, recommend that person as the primary email target and keep the best strategic contact as a LinkedIn/manual-research target.
5. If no individual Verified email exists, mark direct email as blocked and recommend LinkedIn, phone, contact form, referral, or manual enrichment.

Always explain the tradeoff: why the recommended contact is reachable, and whether they are less senior or less ideal than the original strategic target.

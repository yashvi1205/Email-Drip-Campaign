...
- **FastAPI Backend (port `8080` locally, container `8000`)**: Exposes API endpoints, handles webhooks, runs daily follow-ups (Workflow 3), and Gmail reply polling (Workflow 4).
...
```mermaid
graph TD
    Dashboard[React Frontend: port 5173] -->|API Request| API[FastAPI Backend: port 8080]
...
- **The Pitfall**: Running untun on port `8001` (e.g., `untun 8001`) while the local exposed backend runs on port `8080` causes tracking to fail because requests are forwarded to a port where nothing is listening.
...
- **The Correct Setup**:
  1. Always run untun pointing to port **`8080`** (the local exposed FastAPI port):
     ```bash
...
| **Emails sent but clicks/opens aren't tracking** | `LOCAL_BACKEND_URL` is incorrect or untun is on the wrong port. | Run `untun 8080` and ensure the public URL is saved as `LOCAL_BACKEND_URL` in `.env`. |

...
# Phase 0/1: env-only secrets; no credentials.json fallbacks
                        ...
                        updates.append(
                            {
                                ...
                                "values": [
                                    [
                                        str(val).upper() if field_key == "status" else str(val)
                                    ]
                                ],
                            }
                        )
...
def get_latest_post_for_profile(profile_url):
    ...
    try:
        ...
        for record in reversed(all_records):
            ...
            url_in_sheet = (
                norm.get("profile (url)") or norm.get("profile url") or norm.get("profile")
            )
            if normalize_url(url_in_sheet) == normalize_url(profile_url):
                return {
